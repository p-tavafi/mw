...
";
    ...
    protected function preValidateValue($email)
    {
        ...
        $last   = $substr($domain, -1);
        $chars  = $this->getNotAllowedStartEndCharsList();
        if (in_array($first, $chars) || in_array($last, $chars)) {
            return false;
        }

        $first = $substr($username, 0, 1);
        $last  = $substr($username, -1);
        $chars = CMap::mergeArray($chars, array('-', '_'));
        if (in_array($first, $chars) || in_array($last, $chars)) {
            return false;
        }
     ...
     * @return array
     */
    protected function getNotAllowedStartEndCharsList()
    {
        return array(
            '.', '/', '\\', '!', '@', '#', '$', '%', '^',
            '&', '*', '(', ')', '±', '§', '+', '=', '`',
