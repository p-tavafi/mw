...
class List_subscribersController extends Controller
{
    ...
    public function actionIndex($list_uid)
    {
        ...
        $columns[] = array(
            ...
            'field_id'      => null,
            'value'         => null,
            'htmlOptions'   => array('class' => 'empty-options-header options', 'style' => 'width:260px'),
        );
        ...
        foreach ($subscribers as $index => $subscriber) {
            ...
            $actions = array();
            $actions[] = CHtml::link(IconHelper::make('fa-user'), array('list_subscribers/profile', 'list_uid' => $list->list_uid, 'subscriber_uid' => $subscriber->subscriber_uid), array('title' => Yii::t('app', 'Profile info'), 'class' => 'btn btn-primary btn-flat btn-subscriber-profile-info'));
            $actions[] = CHtml::link(IconHelper::make('envelope'), array('list_subscribers/campaigns', 'list_uid' => $list->list_uid, 'subscriber_uid' => $subscriber->subscriber_uid), array('title' => Yii::t('app', 'Campaigns sent to this subscriber'), 'class' => 'btn btn-primary btn-flat'));

            if ($subscriber->getCanBeEdited()) {
                $actions[] = CHtml::link(IconHelper::make('update'), array('list_subscribers/update', 'list_uid' => $list->list_uid, 'subscriber_uid' => $subscriber->subscriber_uid), array('title' => Yii::t('app', 'Update'), 'class' => 'btn btn-primary btn-flat'));
            }
            
            if ($subscriber->getCanBeUnsubscribed() && $subscriber->isConfirmed) {
                $actions[] = CHtml::link(IconHelper::make('glyphicon-log-out'), array('list_subscribers/unsubscribe', 'list_uid' => $list->list_uid, 'subscriber_uid' => $subscriber->subscriber_uid), array('class' => 'btn btn-primary btn-flat unsubscribe', 'title' => Yii::t('app', 'Unsubscribe'), 'data-message' => Yii::t('list_subscribers', 'Are you sure you want to unsubscribe this subscriber?')));
