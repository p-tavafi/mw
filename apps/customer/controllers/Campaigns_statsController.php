...
/**
 * Campaigns_mis_reportsController
 *
...
class Campaigns_statsController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if ($canExport && $filter->isExportAction) {
            ...
            header('Content-Disposition: attachment; filename="campaigns-stats.csv"');
            
            $attributes = array(
                'name', 'listName', 'subscribersCount', 'deliverySuccess', 'uniqueOpens', 
            ...
            foreach ($attributes as $name) {
                $columns[] = $filter->getAttributeLabel($name);
            }
            
            echo implode(",", $columns) . PHP_EOL;

            $criteria = $filter->search()->getCriteria();
            $criteria->select = 'name, campaign_id';
            $criteria->limit  = 10;
            $criteria->offset = 0;
