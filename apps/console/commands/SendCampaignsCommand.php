...
class SendCampaignsCommand extends ConsoleCommand
{
    ...
    protected function prepareEmail($subscriber, $server)
    {
        ...
        $emailPlainText     = null;
        $autoEmailPlainText = false; // 1.3.9.7
        ...
        // since 1.3.9.7
        if (!empty($campaign->option) && $campaign->option->url_tracking == CampaignOption::TEXT_YES && !$autoEmailPlainText && !empty($emailPlainText)) {
            $emailPlainText = CampaignHelper::transformLinksForTracking($emailContent, $campaign, $subscriber, true, true);
        }
